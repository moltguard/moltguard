...
import { evaluateLocal, mostSevere } from "./rules.js";
...
export class OgrClient {
  ...
  // Local hard rules -> runtime -> (unreachable) degraded local decision.
  async decide(event, { localApprover } = {}) {
    if (this.killed) return this._verdict(event, "block", ["kill switch engaged — deny-all"]);
    const hard = evaluateLocal(this.state.policy, event, { hardOnly: true })
      .filter((m) => m.decision === "block");
    if (hard.length) {
      this._spool(event);
      return this._verdict(event, "block", hard.flatMap((m) => m.reasons) || ["local hard rule"], "moltguard.pep.local");
    }
    let resp;
    try {
